      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="no-scrollbar flex gap-1 overflow-x-auto">
          {["all", ...ORDER_STATUSES].map((s) => (
            <Link key={s} href={s === "all" ? "/admin/orders" : `/admin/orders?status=${s}`} className={cn("whitespace-nowrap rounded-full px-3.5 py-1.5 text-sm font-medium capitalize transition", (status ?? "all") === s ? "bg-brand-600 text-white" : "text-neutral-600 hover:bg-white")}>{s}</Link>
          ))}
        </div>
          ...
          <tbody className="divide-y">
            {rows.map((o) => (
              <tr key={o.id} className="hover:bg-neutral-50/60">
                <td className="px-5 py-3"><Link href={`/admin/orders/${o.id}`} className="font-semibold text-brand-700 hover:underline">{o.reference}</Link><p className="text-xs text-neutral-500">{new Date(o.createdAt).toLocaleString("en-NG", { dateStyle: "medium", timeStyle: "short" })}</p></td>
