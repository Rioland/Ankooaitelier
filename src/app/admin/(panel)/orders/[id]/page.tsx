...
export default async function OrderDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <Link href="/admin/orders" className="mb-4 inline-flex items-center gap-1 text-sm text-neutral-500 hover:text-brand-700"><ArrowLeft className="h-4 w-4" /> All orders</Link>
            ...
            {o.items.map((i, k) => (
              <li key={k} className="flex items-center gap-4 px-6 py-4">
                ...
                <div className="flex-1">
                  <Link href={`/product/${i.slug}`} target="_blank" className="font-medium hover:text-brand-700">{i.name}</Link>
