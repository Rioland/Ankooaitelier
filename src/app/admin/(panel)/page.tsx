...
export default async function Dashboard() {
  ...
  const stats = [
    { label: "Revenue", value: naira(Number(rev?.v ?? 0)), icon: Banknote, href: "/admin/orders" },
    { label: "Orders", value: ord?.n ?? 0, icon: ShoppingCart, href: "/admin/orders" },
    { label: "Products", value: prod?.n ?? 0, icon: Package, href: "/admin/products" },
    { label: "Subscribers", value: subs?.n ?? 0, icon: Users, href: "/admin/messages" },
  ];
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.6fr_1fr]">
        <div className="card overflow-hidden">
          <div className="flex items-center justify-between border-b px-6 py-4">
            ...
            <Link href="/admin/orders" className="text-sm font-semibold text-brand-700">View all</Link>
          </div>
          {recent.length === 0 ? (
            ...
            <ul className="divide-y">
              {recent.map((o) => (
                <li key={o.id}>
                  <Link href={`/admin/orders/${o.id}`} className="flex items-center justify-between gap-4 px-6 py-3.5 hover:bg-neutral-50">
                    <div>
        ...
        <div className="card overflow-hidden">
          ...
          {lowStock.length === 0 ? (
            ...
            <ul className="divide-y">
              {lowStock.map((p) => (
                <li key={p.id}>
                  <Link href={`/admin/products/${p.id}`} className="flex items-center justify-between px-6 py-3.5 text-sm hover:bg-neutral-50">
